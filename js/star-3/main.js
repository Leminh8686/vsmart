...
main = function () {
    ...
    function onScroll() {
        ...
        var sec2Pos = section2.position().top + $("#thong-tin").position().top;
        var sec3Pos = section3.position().top + $("#thong-tin").position().top;
        var sec4Pos = section4.position().top + $("#thong-tin").position().top;
        var sec5Pos = section5.position().top;

        var sec8Pos = section8.position().top;
        var sec6Pos = section6.position().top + sec8Pos;
        var sec7Pos = section7.position().top + sec8Pos;
        ...
        /* console.log(scrollPos+" xxx "+  (sec10Pos - sec10H / 4  )) */
        if (scrollPos > sec1Pos && scrollPos < sec1Pos + sec1H / 3 && !anim1) {
            anim1 = true;
            if (cameraRoot) {
                cameraRoot.children[0].gotoAndPlay(0);
            }
            /* gsap.to(".kv-logo", { y: 0, alpha: 1, duration: .5 }) */
            gsap.timeline().to(".flare", { scale: 1, alpha: 1, rotation: 360, duration: 0.85, delay: 0.5 })
            /* .to(".flare",{alpha:0,rotation:720,duration:0.85}) */
        }
        if (scrollPos > sec1H + sec1Pos) {

            anim1 = false;
            /* gsap.set(".kv-logo", { y: -50, alpha: 0 }) */
            gsap.set(".flare", { scale: .1, rotation: 0, alpha: 0 })
        ...
        if (scrollPos > sec2Pos - sec2H / 4 && scrollPos < sec2Pos + sec2H / 3 && !anim2) {
            anim2 = true;
            console.log("ccc");
            gsap.to(".s2-phone", { x: 0, y: 0, alpha: 1, duration: .75 })

        }
        if (scrollPos > sec2H + sec2Pos || scrollPos < sec2Pos - sec2H / 2) {
            anim2 = false;
            gsap.set(".s2-phone", { x: -30, y: 80, alpha: 0 })

        }
        //3
        if (scrollPos > sec3Pos - sec3H / 4 && scrollPos < sec3Pos + sec3H / 3 && !anim3) {
            anim3 = true;
            gsap.to(".s3-p1", { scale: 1, alpha: 1, delay: 0 })
            gsap.to(".s3-p2", { scale: 1, alpha: 1, delay: .25 })
            gsap.to(".s3-p3", { scale: 1, alpha: 1, delay: 0.5 })
            gsap.to(".s3-p4", { scale: 1, alpha: 1, delay: 0.75 })
        }
        if (scrollPos > sec3H + sec3Pos || scrollPos < sec3Pos - sec3H) {
            anim3 = false;
            gsap.to(".s3-p1", { scale: 1.5, alpha: 0 })
            gsap.to(".s3-p2", { scale: 1.5, alpha: 0 })
            gsap.to(".s3-p3", { scale: 1.5, alpha: 0 })
            gsap.to(".s3-p4", { scale: 1.5, alpha: 0 })
        }
        //4
        ...
        if (scrollPos > sec5Pos - sec5H / 4 && scrollPos < sec5Pos + sec5H / 3 && !anim5) {
            anim5 = true;
            gsap.timeline().to(".s5-city", { opacity: 1, duration: 0.3, ease: Linear.easeNone }).to(".s5-city", { scaleX: 1.0, scaleY: 1.0, duration: 1, ease: Linear.easeNone });
            gsap.timeline().to(".s5-vector", { opacity: 1, duration: 0.3, delay: 0.5 }).to(".s5-vector", { scale: 1, duration: 1, ease: Power4.easeOut });
        }
        if (scrollPos > sec5H + sec5Pos || scrollPos < sec5Pos - sec5H) {

            anim5 = false;
            gsap.set(".s5-city", { opacity: 0, scaleX: 1.2, scaleY: 1.2 });
            gsap.set(".s5-vector", { opacity: 0, scale: .8 });
        }
        //6

        if (scrollPos > sec6Pos - sec6H / 4 && scrollPos < sec6Pos + sec6H / 3 && !anim6) {
            anim6 = true;
            gsap.to(".s9-phone-1", { y: 0, x: 0,  duration: 1 })

        }
        if (scrollPos < sec6Pos - sec6H && anim6 || scrollPos >= sec7Pos && anim6) {
            anim6 = false;
            gsap.set(".s9-phone-1", { y: -50, x: -20 })

        }
        //7
        if (scrollPos > sec7Pos - sec7H / 4 && scrollPos < sec7Pos + sec7H / 3 && !anim7) {
            anim7 = true;
            gsap.to(".s9-phone-2", { y: 0, x: 0,  duration: 1 })
        }
        if (scrollPos < sec6Pos - sec6H && anim7 || scrollPos >= sec9Pos && anim7) {
            anim7 = false;
            gsap.set(".s9-phone-2", { y: 50, x: -20 })
        ...
        if (scrollPos < sec9Pos - sec9H || scrollPos > sec9Pos + sec9H) {
            anim9 = false;
        ...
        if (scrollPos > sec10Pos - sec10H / 4 && scrollPos < sec10Pos + sec10H / 3 && !anim10) {
            anim10 = true;
            gsap.to($("#left1"), { x: 0,  duration: 0.8, ease: Linear.easeNone })
            gsap.to($("#left2"), { x: 0,  duration: 1, ease: Linear.easeNone })
            gsap.to($("#right1"), { x: 0,  duration: 0.8, ease: Linear.easeNone })
            gsap.to($("#right2"), { x: 0, duration: 1, ease: Linear.easeNone })
        }
        if (scrollPos < sec10Pos - sec10H || scrollPos > sec10Pos + sec10H) {
            anim10 = false;
            gsap.set($("#left1"), {x: $("#left1").width()  })
            gsap.set($("#left2"), {x: $("#left1").width() * 2 - 9 })
            gsap.set($("#right1"), {  x: -$("#right1").width() })
            gsap.set($("#right2"), {  x: -$("#right1").width() * 2 })
        }
    ...
    var section4 = $(".sec4");
    var section4 = $(".sec4");
    var section5 = $("#goc-rong")
    var section6 = $(".s9-phone-1")
    var section7 = $(".s9-phone-2")
    var section8 = $("#pin-vantay")
    var section9 = $("#cpu")
    var section10 = $("#vos")
